...
class Encoder(nn.Module):
    ...
    def forward(self, image_list, lidar_list, maps_list, velocity):
        '''
        ...
        map_tensor   = torch.stack(maps_list,  dim=1).view(bz * self.config.seq_len, map_channel, h, w)
        ...
        map_features = self.img_map_encoder.features.conv1(map_tensor)
        map_features = self.img_map_encoder.features.bn1(map_features)
        map_features = self.img_map_encoder.features.relu(map_features)
        map_features = self.img_map_encoder.features.maxpool(map_features)
        ...
        map_features   = self.img_map_encoder.features.layer1(map_features)
...
class MMFN(nn.Module):
    ...
    def forward(self, image_list, lidar_list, maps_list, vectormaps_list, radar_list, radar_adj, target_point, velocity):
        ...
        Args:
            ...
            velocity (tensor): input velocity from speedometer
        '''
        fused_features = self.encoder(image_list, lidar_list, maps_list, velocity)
