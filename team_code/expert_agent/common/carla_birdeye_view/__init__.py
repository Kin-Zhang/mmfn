...
from .cache import generate_opendrive_content_hash
...
class BirdViewProducer:
    ...
    def produce(self, agent_tf: carla.Transform, bbxy: np.ndarray) -> BirdView:
        agent_vehicle_loc = agent_tf.location
        ...
        masks = self._render_actors_masks(agent_tf, bbxy, masks)
        cropped_masks = self.apply_agent_following_transformation_to_masks(agent_tf, masks)
    ...
    def _render_actors_masks(
        ...
        """
        masks[BirdViewMasks.AGENT.value] = self.masks_generator.agent_vehicle_mask(agent_tf, bbxy)
        return masks

    def apply_agent_following_transformation_to_masks(
        self, agent_tf: carla.Transform, masks: np.ndarray
    ) -> np.ndarray:
        """Returns image of shape: height, width, channels"""
        agent_transform = agent_tf
        angle = (
