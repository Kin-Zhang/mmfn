...
parser.add_argument('--id', type=str, default='new_transfuser', help='Unique experiment identifier.')
...
if __name__ == '__main__':
	...
	# wandb.init(project="mmfn", entity="kinzhang", config = args, mode=args.wandb_mode)
	wandb.watch(model)
	...
	for epoch in range(trainer.cur_epoch, args.epochs):
		...
		if epoch % args.val_every == 0: 
			trainer.validate(dataloader_train)
			if epoch % args.save_every == 0:
				trainer.save()

	wandb.save(os.path.join(args.logdir, 'best_model.pth'))
	print("TOTAL RUNNING TIME: --- %s hours ---" % round((time.time() - start_time)/3600,2))
	wandb.finish()
